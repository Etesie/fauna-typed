...
export function parseFaunaType(signature: string, mode: ParseMode): ParsedResult {
  ...
  if (trimmed.startsWith('Array<') && trimmed.endsWith('>')) {
    ...
    if (mode !== 'main' && inner.type.startsWith("createRef(")) {
  ...
  if (trimmed.startsWith('Ref<') && trimmed.endsWith('>')) {
    ...
    if (mode === 'main') {
      // In main mode we output the lowercased reference key (as in the new output)
      return { type: inside.toLowerCase(), isOptional };
    } else {
      // In create/update/replace modes, wrap with createRef(...)
      return { type: `createRef(type("'${inside}'"))`, isOptional };
    }
  }
  ...
  // 7) Fallback: just return the trimmed signature
